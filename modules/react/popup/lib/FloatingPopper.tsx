...
export interface FloatingPopperProps {
  ...
  children: ((props: {placement: FloatingUIPlacement}) => React.ReactNode) | React.ReactNode;
  getAnchorClientRect?: () => DOMRect;
  open?: boolean;
  placement?: FloatingUIPlacement;
  fallbackPlacements?: FloatingUIPlacement[];
  onPlacementChange?: (placement: FloatingUIPlacement) => void;
  portal?: boolean;
...
function isRenderProp(
  children: any
): children is (props: {placement: FloatingUIPlacement}) => React.ReactNode {
...
const OpenFloatingPopper = React.forwardRef<HTMLDivElement, FloatingPopperProps>(
  (
    ...
    const [placement, setPlacement] = React.useState<FloatingUIPlacement>(initialPlacement);
      ...
      const updatePosition = () => {
        computePosition(anchorEl, stackRef.current!, {
          ...
          if (computedPlacement !== placement) {
            setPlacement(computedPlacement as FloatingUIPlacement);
            onPlacementChange?.(computedPlacement as FloatingUIPlacement);
          }
        });
      };
